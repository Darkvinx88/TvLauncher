    ...
    def _set_initial_category(self):
        ...
        for i, category in enumerate(self.categories):
            if category.get('is_default', False):
                ...
                return
        # Se nessuna categoria è impostata come default, resta su "All" (indice 0)
...
def integrate_categories(launcher):
    ...
    # NUOVO: Ricostruisci il carosello con il filtro della categoria iniziale
...
def add_category_joystick_support(launcher):
    """Registra il category selector nel JoystickManager"""
...
def _handle_category_selector_joystick_blocking(launcher):
    """ Gestione category selector - LEFT/RIGHT naviga, DOWN chiude + D-PAD SUPPORT"""
    import pygame
    from PyQt6.QtCore import QTimer
    ...
    # PULSANTE B - chiude SOLO il selector
    ...
    if current_b_state and not last_b_state:
        launcher.sound_manager.back()
        _close_category_selector_with_block(launcher)
        #  Consuma il pulsante B per evitare che venga rilevato dopo
    ...
    #  Consuma TUTTI gli altri pulsanti durante la navigazione
...
def _close_category_selector_with_block(launcher):
    ...
    # Forza TUTTI gli stati come TRUE se premuti

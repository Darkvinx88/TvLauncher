...
class AppTile(QWidget):
    def __init__(self, app_data, scaling, parent=None):
        ...
        self.glow_effect = None  # ✅ AGGIUNTO
    ...
    def set_focused(self, focused):
        ...
        if focused:
            ...
            # ✨ NUOVO: Controlla se il glow è attivo
...
class TVLauncher(QMainWindow):
    def __init__(self):
        ...
        print("✅ Volume control initialized (waiting for joystick)")
...
# ===================================
    ...
    def detect_joystick(self):
        # SE abbiamo già un joystick attivo, NON fare nulla
        ...
        try:
            ...
            if count > 0:
                self.joystick = pygame.joystick.Joystick(0)
                self.joystick.init()
                print(f"🎮 Joystick reconnected: {self.joystick.get_name()}")
                
                # ✅ Riavvia timer polling
                ...
                # ✅ NOTIFICA VOLUME MANAGER (solo qui, non in poll_joystick)
                if self.volume_manager:
                    self.volume_manager.set_joystick(self.joystick)
                    print("   └─ Volume control reactivated")
    ...
    def poll_joystick(self):
        ...
        try:
            # Controlla volume mode PRIMA di tutto
            if hasattr(self, 'volume_manager') and self.volume_manager and \
               self.volume_manager.volume_mode_active:
    ...
    def check_launched_process(self):
        ...
        try:
            ...
            # ✅ NUOVO: Controlla anche lo stato del processo
            ...
            # Debug: mostra che il processo è ancora vivo
            # Commentalo se crea troppo spam nel log
            # print(f"✓ Process {self.launched_process} still running (status: {status})")
    ...
    def check_window_focus(self):
        """✅ FIXED: Controlla se la finestra del launcher ha riacquistato il focus (Linux)"""
        if not IS_LINUX:
            return
        ...
        if not self.inputs_enabled:
            ...
            if self.isActiveWindow():
                print(f"🔍 Launcher regained focus - app closed")
                self.on_app_closed()       
   
    def on_app_closed(self):
        """✅ FIXED: Chiamato quando l'app lanciata viene chiusa"""
        ...
        # ✅ RIMOSSO: Non chiamare force_window_focus qui!
        # Il window manager si occupa già di tutto
    ...
    def force_window_focus(self):
        """
        ✅ DEPRECATED: Questo metodo non dovrebbe più essere chiamato
        Il window manager gestisce tutto il ripristino
        
        Mantenuto per compatibilità ma non fa nulla di importante
        """
        print("⚠️ force_window_focus called (deprecated - window manager handles this)")
    ...
    def _linux_force_focus(self):
        """
        ✅ DEPRECATED: Non più necessario
        Il window manager ora gestisce tutto con _ensure_foreground_linux
        """
        pass  # Non fa nulla 
    ...
    def save_config(self):
        ...
        for app in self.apps:
            ...
            if isinstance(clean_app['icon'], QPixmap):
    ...
    def launch_current_app(self):
        """✅ FIXED: Lancia l'app con tracking corretto del processo"""
        if not self.apps:
            return
        ...
        try:
            ...
            # ✅ FIX LINUX: Usa Popen con parametri corretti
            process = subprocess.Popen(
                app['path'],
                shell=True,
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                # ✅ IMPORTANTE: start_new_session invece di preexec_fn
                # Più affidabile su Linux moderno
            ...
            # ✅ FIX CRITICO: Avvia ENTRAMBI i timer di controllo
            
            # Timer 1: Controlla se il processo esiste ancora
            if self.process_check_timer:
                self.process_check_timer.stop()
            
            self.process_check_timer = QTimer()
            self.process_check_timer.timeout.connect(self.check_launched_process)
            self.process_check_timer.start(1000)  # Ogni 1 secondo
            print("   ✅ Process check timer started")
            
            # Timer 2: Su Linux, controlla anche il focus della finestra
            if IS_LINUX:
                if self.focus_check_timer:
                    self.focus_check_timer.stop()
                ...
                self.focus_check_timer.start(500)  # Ogni 0.5 secondi
                print("   ✅ Focus check timer started (Linux)")
    ...
    def keyPressEvent(self, event: QKeyEvent):
        # ✅ PRIMA: Controlla se il settings menu è aperto
        if hasattr(self, 'settings_menu') and getattr(self.settings_menu, 'is_open', False):
            self.settings_menu.keyPressEvent(event)
            return
        
        # ✅ SECONDA: Controlla se la ricerca è aperta
        if hasattr(self, 'quick_search') and self.quick_search.isVisible():
